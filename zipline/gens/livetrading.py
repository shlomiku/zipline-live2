...
log = Logger('Live Trading')


class RealtimeClock(object):
    def __init__(self, time_skew,
                 sessions,
                 execution_opens,
                 execution_closes,
    ...
    def __iter__(self):
        ...
        while True:
            ...
            server_time = (current_time + self.time_skew).floor('1min')
            ...
            if server_time == self.before_trading_start_minutes and not self._before_trading_start_bar_yielded:
                ...
                log.error('Strange condition @ {}', server_time)
                return


class AlgorithmLiveExecutor(object):
    ...
    def transform(self):
        ...
        with ExitStack() as stack:
            ...
            for dt, action in self.clock:
                log.info('Processing tick: %s - %s' % (dt, action))
                if action == BAR:
